...
- ADR-0030 — Integrated editor; `#EditorState` is a parallel sum type for editor-specific async
  states.
- ADR-0071 — Skeleton mandate for resource list views — extends the skeleton vocabulary established
  here to all non-workload resource list views; bans `ProgressView` for initial list load.
