// Views/Resources/Config/RuntimeClassesListView.swift — app_shell bounded context
// DDD role: View
...
// MARK: - RuntimeClassesListView

/// Table view listing Kubernetes RuntimeClasses (cluster-scoped).
@MainActor
public struct RuntimeClassesListView: View {

    let clusterId: ClusterId
    @State private var viewModel = RuntimeClassesListViewModel()
    ...
    public var body: some View {
        ...
        .navigationTitle("RuntimeClasses")
        .toolbar { toolbarContent }
        .task { await viewModel.start(clusterId: clusterId) }
        .alert("Delete RuntimeClass?", isPresented: $viewModel.showDeleteConfirm) {
    ...
    private func errorView(_ error: Error) -> some View {
        ErrorStateView(
            title: "Failed to load RuntimeClasses",
            error: error,
            retryable: true,
    ...
    private var tableView: some View {
        Table(viewModel.rows, selection: $viewModel.selectedId) {
            ...
            TableColumn("Handler", value: \.handler)
        ...
        .contextMenu(forSelectionType: UUID.self) { ids in
            if let row = viewModel.rows.first(where: { ids.contains($0.id) }) {
                let actions = RowActionMenuBuilder.actions(for: RowActionContext(
                    kind: "RuntimeClass", name: row.name, family: .config
                ))
