// Views/Resources/Config/MutatingWebhooksListView.swift — app_shell bounded context
// DDD role: View
...
// MARK: - MutatingWebhooksListView

/// Table view listing Kubernetes MutatingWebhookConfigurations (cluster-scoped).
@MainActor
public struct MutatingWebhooksListView: View {

    let clusterId: ClusterId
    @State private var viewModel = MutatingWebhooksListViewModel()
    ...
    public var body: some View {
        ...
        .navigationTitle("MutatingWebhookConfigurations")
        .toolbar { toolbarContent }
        .task { await viewModel.start(clusterId: clusterId) }
        .alert("Delete MutatingWebhookConfiguration?", isPresented: $viewModel.showDeleteConfirm) {
    ...
    private func errorView(_ error: Error) -> some View {
        ErrorStateView(
            title: "Failed to load MutatingWebhookConfigurations",
            error: error,
            retryable: true,
        ...
        .contextMenu(forSelectionType: UUID.self) { ids in
            if let row = viewModel.rows.first(where: { ids.contains($0.id) }) {
                let actions = RowActionMenuBuilder.actions(for: RowActionContext(
                    kind: "MutatingWebhookConfiguration", name: row.name, family: .config
                ))
